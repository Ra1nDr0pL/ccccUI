package com.rain.newqq.data

import android.content.Context
import android.content.SharedPreferences
import com.github.kyuubiran.ezxhelper.init.InitFields.appContext
import com.rain.newqq.BuildConfig
...
object ConfigManager {
    ...
    val sPrefs by lazy {
        appContext.getSharedPreferences(BuildConfig.APPLICATION_ID, Context.MODE_PRIVATE)//调用宿主Context所以它会直接存在宿主的data/data/包名里
    }
    ...
    fun putString(key: String, value: String) {
        sPrefs.edit().putString(key, value).apply()//写入
    }

    fun getString(key: String, defValue: String = ""): String {
        return sPrefs.getString(key, defValue) ?: defValue//获取
    }
    ...
    fun modulestop(): Boolean{
        return getBool("modulestop",false)
    }
/*
    ...
    fun getInstance(): ConfigManager? {
        synchronized(ConfigManager) {
            ...
            if (sharedPreferences.get() == null) {
                sharedPreferences = WeakReference(
                    sPrefs
                )
            } //这里在写入SP，在自己的包名           这里的getContext返回自定义变量，mContext
        }
